...
SELECT 
 ...
 -- Maior venda
SELECT venda_id, ROUND(SUM(quantidade * valor_unitario), 2) AS maior_venda
FROM vendas_itens2
GROUP BY venda_id
...
-- Menor venda
SELECT venda_id, ROUND(SUM(quantidade * valor_unitario), 2) AS menor_venda
FROM vendas_itens2
GROUP BY venda_id
...
--valor medio por unidade considerando o total vendido
SELECT 
    produto_id,
    ROUND(SUM(quantidade * valor_unitario) / SUM(quantidade), 2) AS media_ponderada_valor_unitario
FROM 
    vendas_itens2
GROUP BY 
    produto_id
ORDER BY 
    media_ponderada_valor_unitario DESC;
